...
- **Global:** 2,197,593 total cases, and 153,090 deaths in 206 countries with reported COVID-19 cases.
- **EU28:** 893,921, and 93,152 deaths.
- **Belgium:** 36,578 total cases, 5,069 total hospitalised, 1,119 in intensive care, 5,453 deceased, and 8,348 recovered.
...
We now start with the analysis for Belgium. Figure 1 shows total COVID numbers for cases, hospitalizations and deceased patients by date.[^1]
...
There are currently 36,578 confirmed infections, 5,069 hospitalized patients, 1,119 patients in intensive care, with 809 patients on respiratory aid, and 5,453 people have died from the infection. On the up side, 8,348 patients have been released from the hospital. There is a current maximum capacity of 2,293 ICU beds reserved for COVID patients, and 48.80% of capacity of ICU beds is taken.
...
{% include plots/daily-update-2020-04-18/2020-04-18-Belgium-cumulative.html %}
...
{% include plots/daily-update-2020-04-18/2020-04-18-Belgium-province.html %}

<p style="text-align: center; font-style: italic;">Figure 2: Number of cases and hospitalizations by province.</p>

Next, Figure 3 shows the number of cases by municipality.[^3] The city with the highest number of infections is currently Antwerpen with 1,313. Liège follows with 826 and Brussel is third with 544 cases.
...
[^4]: The geographic data used to build the map are from 2014. For the Belgian municipalities that merged in 2019 we include the former municipalities (pre-2019) separately, and we impute the cases for the merged municipality in 2019 to each pre-2019 municipality. Some of the polygons in the map are empty due to missing data for that municipality or a problem with the geographic data in the geojson file. For example, Elsene (Ixelles) is not on the map, but we know that it has 260 cumulative cases, that is 0.3% of its population is infected.

{% include plots/daily-update-2020-04-18/2020-04-18-Belgium-map.html %}

<p style="text-align: center; font-style: italic;">Figure 3: Number of cases by municipality.</p>

We also compare COVID mortality to expected deaths for a given day in Figure 4. The figure plots the daily expected number of deaths (blue curve) as the average number of deaths of 2015-2017. The orange curve is the 7-day moving average. The  green curve plots the number of deaths from COVID, together with its 7-day MA (red). Excess mortality is then the number of observed deaths minus the number of expected deaths.

While the data is incomplete, as we have to wait for general mortality data for 2020, the figure already shows a telling result: <mark class="text-black" style="background-color: #4dffd2;">deaths from only COVID are in the same order of magnitude as total expected deaths across the whole population. This implies any other causes of deaths will contribute to excess mortality for this period.</mark> Most likely, the source of excess mortality is COVID. Even if reports are over-counting COVID deaths by including “plausible COVID deaths” from co-morbidities and pre-existing conditions, the over-counting would have to be relatively large to avoid excess mortality for this period.

{% include plots/daily-update-2020-04-18/2020-04-18-Belgium-average-deaths.html %}
...
In Figure 5, we compare the life expectancy for Belgium (blue) against COVID deaths (red) across age groups.
...
<mark class="text-black" style="background-color: #4dffd2;">Turning to COVID mortality, we see that the distribution for both sexes is skewed towards the elderly, with a turning point at around 70 years.</mark>

More elderly people die from COVID than predicted by the life expectancy tables, relative to other age groups. Conversely, younger people have a higher probability of dying from another cause than one allocated to COVID. If COVID would not discriminate across age, both curves would coincide. Importantly, this curve might change over the course of the disease. For instance if elderly people die faster, but also younger people die over the epidemic, the distribution might shift its gravitational point to younger ages later in the epidemic. Importantly, this data does not account for pre-existing conditions, some of which might be a plausible risk factor for COVID (diabetes, vascular diseases etc.): age and pre-existing conditions correlate strongly, and without more information, we cannot disentangle both causes of mortality. Still, there can be over-mortality in the population due to COVID even if COVID is not the direct cause of death, for example pre-existing conditions, an overload of the health case system that generates otherwise preventable deaths etc.
...
{% include plots/daily-update-2020-04-18/2020-04-18-Belgium-life-expectancy.html %}

<p style="text-align: center; font-style: italic;">Figure 5: Life expectancy versus distribution of COVD deaths by age groups and region.</p>
...
{% include plots/daily-update-2020-04-18/2020-04-18-MA_ln_share_cases-midpoint-Belgium.html %}
...
When looking at the EU28, the country with the most cases is Spain with 188,068 total confirmed cases.Second is Italy, with 172,434 confirmed cases, and Germany follows, with 137,439 cases. It's worth noting that Sweden's infection rate has decreased significantly since around the 9th day after the 100th confirmed case, while having a [more lenient policy](https://www.forbes.com/sites/davidnikel/2020/03/30/why-swedens-coronavirus-approach-is-so-different-from-others/#538677cb562b).  

The Figure also reports the total number of deaths since the 10th death. As noted in the introduction, both the number of cases and deaths are prone to several sources of non-random measurement error. However, we expect the measurement error to be smaller for the reported number of deaths. As of today, the country with the most deaths is United States of America, with 37,054 deaths. Second is Italy, with 22,747 deaths, and Spain follows, with 19,478 deaths. These top 3 countries account for 0.52% of all deaths worldwide. Again, we see that China has stabilized for now, with around 3,500 deaths. We repeat the graph for the 10 highest mortality countries in the EU28. The EU28 country with the most deaths is Italy, with 22,747 deaths. Second is Spain, with 19,478 deaths, and France follows, with 18,681 deaths.

{% include plots/daily-update-2020-04-18/2020-04-18-ln_cum_cases-ln_cum_deaths-after_10th.html %}
...
Countries around the world have taken several measures to slow down the speed of the epidemic in their country. The strength and timing of these policies however, varies significantly across countries. Figure 9 shows an index of the strength of measures taken, and at what moment in the outbreak, expressed as the total number of deaths at that moment. The index is [The Oxford COVID-19 Government Response Tracker](https://www.bsg.ox.ac.uk/research/research-projects/oxford-covid-19-government-response-tracker),[^8] ranges between 0 (no policy) to 100 (maximum measures) and combines 13 indicators of government responses, including school closures, travel bans, and fiscal or monetary measures.
...
{% include plots/daily-update-2020-04-18/2020-04-18-policy.html %}
...
{% include plots/daily-update-2020-04-18/2020-04-18-MA_cases-MA_deaths-after_100th.html %}
...
{% include plots/daily-update-2020-04-18/2020-04-18-MA_ln_share_cases-MA_ln_share_deaths-midpoint.html %}
